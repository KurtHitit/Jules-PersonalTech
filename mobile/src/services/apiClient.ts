...
const API_BASE_URL = "http://10.0.2.2:3000/api"; // Adjust as needed for your setup
...
apiClient.interceptors.request.use(
  async (config) => {
    ...
    if (token) {
      ...
      console.log(
        "[APIClient] Token added to request headers for URL:",
        config.url
      );
    } else {
      console.log(
        "[APIClient] No token found, request sent without Authorization header for URL:",
        config.url
      );
    }
    return config;
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    console.error(
      "[APIClient] Error in response for URL:",
      error.config?.url,
      "Status:",
